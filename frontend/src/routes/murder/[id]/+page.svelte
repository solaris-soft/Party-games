...
<div class="min-h-screen bg-black relative overflow-hidden">
	...
	<div class="relative z-10 px-4 py-8 max-w-4xl mx-auto">
		...
		{#if eliminationResult}
			<div
				class="bg-black/80 border-2 {eliminationResult.wasMurderer
					? 'border-green-500'
					: 'border-red-500'} text-white px-6 py-4 rounded-lg mb-4 text-center font-['Creepster'] text-2xl animate-fade-in"
			>
				{eliminationResult.eliminatedPlayer} was {eliminationResult.wasMurderer
					? 'the murderer!'
					: 'not the murderer...'}
			</div>
		{/if}
		...
		<div class="mb-8">
			<h1
				class="text-5xl md:text-6xl font-['Creepster'] text-center text-red-600 mb-4 drop-shadow-[0_0_15px_rgba(255,0,0,0.3)] psychological-title"
			>
				Murder Game
			</h1>
			{#if isMurderer}
				<div
					class="bg-red-900/50 border-2 border-red-500 text-red-200 px-6 py-3 rounded-lg mb-4 text-center font-['Creepster'] text-2xl animate-pulse"
				>
			...
			<div class="my-6 text-center">
				<div
					class="inline-block bg-black/70 border-2 border-red-500 rounded-lg px-8 py-3 transform hover:scale-105 transition-transform duration-200"
				>
					<p class="text-gray-400 text-sm mb-1">Current Phase</p>
					<p
						class="font-['Creepster'] text-3xl text-red-500 drop-shadow-[0_0_10px_rgba(255,0,0,0.3)] phase-text"
					>
		...
		<div class="mb-8">
			<h2
				class="text-2xl font-['Creepster'] text-red-500 mb-6 text-center drop-shadow-[0_0_10px_rgba(255,0,0,0.2)]"
			>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{#each players as player}
					<div
						class="border border-red-900/50 rounded-lg p-4 bg-black/50 backdrop-blur-sm transition-all duration-200 {!player.isAlive
							? 'opacity-50'
							: 'hover:border-red-500/50 hover:shadow-[0_0_15px_rgba(255,0,0,0.1)]'}"
					>
						<div class="flex justify-between items-start mb-2">
							<h3 class="font-['Creepster'] text-xl text-red-400">{player.name}</h3>
							{#if player.ready}
								<span class="bg-green-900/50 text-green-300 text-xs px-2 py-1 rounded-full"
									>Ready</span
								>
							{/if}
						</div>
