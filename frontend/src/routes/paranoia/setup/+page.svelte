...
<div class="container" in:fly={{ y: 20, duration: 1000, delay: 200 }}>
    <h1 class="text-6xl font-bold text-red-400 tracking-widest uppercase relative {glitchActive ? 'animate-glitch' : ''} drop-shadow-[0_0_10px_rgba(255,0,0,0.5)]">
        Enter Your Identity
    </h1>
    <form onsubmit={handleSubmit} class="form">
        <div class="form-group">
            <label for="name" class="text-2xl text-red-500/70 tracking-wider">What's your name, citizen?</label>
            <input
                type="text"
                id="name"
                bind:value={name}
        ...
        <button type="submit" class="button bg-red-500/20 hover:bg-red-500/30 border border-red-500/50 text-red-400">
            Initialize Identity
        </button>
    </form>
</div>
