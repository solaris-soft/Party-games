...
{#if showContent}
  ...
  >
    ...
    <div class="space-y-6 sm:space-y-8">
      ...
      <ul class="space-y-4 sm:space-y-6 font-['IBM+Plex+Mono'] text-xs sm:text-sm tracking-wide max-w-2xl mx-auto">
        {#each [
          { icon: '📝', text: 'Each player enters 5 secret things about themselves' },
          { icon: '🎯', text: 'Each round consists of one secret being revealed' },
          { icon: '👥', text: 'Players vote on who they think the secret belongs to' },
          { icon: '✅', text: 'If the majority guesses correctly, the person who the secret belongs to drinks' },
          { icon: '❌', text: 'If the majority guesses wrong, everyone drinks and the real answer remains hidden' },
          { icon: '🎲', text: 'The game continues until all secrets are revealed' },
          { icon: '🏆', text: 'The player who survives the most drinks wins' },
          { icon: '⚠️', text: 'Remember: honesty is key - no lying about your secrets!' }
        ] as rule, i}
          <li 
          ...
          >
            <div class="absolute left-0 top-2 w-1.5 sm:w-2 h-1.5 sm:h-2 bg-green-800/30 rounded-full"></div>
            <span class="text-lg sm:text-xl group-hover:scale-110 transition-transform">{rule.icon}</span>
